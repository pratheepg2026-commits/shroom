...
/** Payload we send from ReturnForm -> handleReturn */
type SalesReturnDraft = {
  returnedProducts: { productId: string; quantity: number; price: number }[];
  warehouseId: string;
  date: string;
...
interface ReturnFormProps {
  ...
  allProducts: Product[];
  onReturn: (updatedSale: Sale | WholesaleSale, salesReturnData: SalesReturnDraft) => void;
  onCancel: () => void;
}
...
const ReturnForm: React.FC<ReturnFormProps> = ({ sale, allReturnsForSale, onReturn, allProducts, onCancel }) => {
  ...
  const [reason, setReason] = useState<string>(''); // ✅ NEW: reason input state
  ...
  const getAvailableToReturnQty = useCallback(
  ...
  const handleAddProductToReturn = () => {
    ...
    if (existingIndex > -1) {
      ...
      setReturnedProducts([
        ...returnedProducts,
        { name: selectedProduct, quantity: currentQty, price: currentPrice },
      ]);
    }

    setCurrentQty(1);
    const nextProduct = originalProductsList.find(
      p => getAvailableToReturnQty(p.name) > 0 && p.name !== selectedProduct
    );
    if (nextProduct) {
  ...
  const handleSubmit = () => {
    ...
    if (!reason.trim()) {
      alert('Please enter a reason / description for this return.');
      return;
    }

    const newProductsForSale = sale.products
      .map(p => {
    ...
    <div className="space-y-4">
      <div>
        <h4 className="font-semibold text-white">
          Returning items for Invoice: {sale.invoiceNumber}
        </h4>
        <p className="text-sm text-gray-400">
          Customer:{' '}
          {'customerName' in sale ? sale.customerName : sale.shopName}
        </p>
      </div>
      ...
      <div className="p-3 bg-gray-800/50 rounded-lg space-y-2">
        <div className="space-y-3">
          <div>
            ...
            >
              {originalProductsList.map(p => (
                <option key={p.name} value={p.name}>
                  {p.name} (Bought: {p.quantity})
                </option>
              ))}
            </select>
          </div>
                ...
                <tr key={p.name} className="border-t border-white/10">
                  ...
                  <td className="py-2 text-right">
                    <Button
                      ...
                      onClick={() => handleRemoveProductFromReturn(p.name)}
                    >
                      <svg
      ...
      <div>
        <label className="block text-xs font-medium text-gray-400 mb-1">
          Reason / Description for Return
        </label>
        <textarea
          value={reason}
          onChange={e => setReason(e.target.value)}
          placeholder="Enter reason for this sales return..."
          className="w-full bg-gray-800/50 border border-white/20 rounded-md p-2 text-gray-200 min-h-[80px]"
        />
      </div>
...
const SalesReturn: React.FC = () => {
  ...
  const getProductName = (productId: string) => {
    ...
    const product = allProducts.find(
      p => p.id === productId || (p as any).productId === productId || (p as any).product_id === productId
    );
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      let combined: CombinedSale[] = [
        ...salesData.map((s: Sale) => ({ ...s, type: 'Retail' as const })),
        ...wholesaleData.map((w: WholesaleSale) => ({ ...w, type: 'Wholesale' as const })),
      ];

      // ✅ Sort sales: latest date on top
      combined = combined.sort(
        (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()
      );

      // ✅ Sort returns: latest date on top
      const sortedReturns: SalesReturn[] = [...returnsData].sort(
        (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()
      );
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    // Keep order (allSales already sorted) – but explicitly sort again if you want:
  ...
  const handleReturn = async (updatedSaleData: Sale | WholesaleSale, salesReturnData: SalesReturnDraft) => {
    try {
      const completeReturnData = {
        saleId: (updatedSaleData as any).id,
        warehouseId: (updatedSaleData as any).warehouseId || 'default',
        returnedProducts: salesReturnData.returnedProducts.map(p => ({
          productId: p.productId,
          quantity: p.quantity,
          price: p.price,
        })),
        totalRefundAmount: salesReturnData.totalRefundAmount,
        date: salesReturnData.date || new Date().toISOString().split('T')[0],
        reason: salesReturnData.reason, // ✅ send reason to backend
      };
      ...
      const saleUpdatePayload = {
        id: (updatedSaleData as any).id,
        products: updatedSaleData.products,
        totalAmount: updatedSaleData.totalAmount,
        warehouseId: (updatedSaleData as any).warehouseId,
      };

      if ((updatedSaleData as any).type === 'Retail') {
    ...
    try {
      const flattenedData = returns.flatMap(ret =>
        ret.returnedProducts.map(product => ({
          ...
          reason: (ret as any).reason || '', // include reason in CSV if needed
        }))
      );
  ...
  return (
    <div>
      ...
      <div className="bg-black/20 backdrop-blur-md border border-white/10 rounded-xl shadow-lg p-4 mb-6">
        ...
        >
          <select
            value={searchType}
            onChange={e =>
              setSearchType(e.target.value as 'invoice' | 'customer' | 'date')
            }
      ...
      {/* Sales list (for initiating returns) */}
                ...
                >
                  ...
                  <td className="px-6 py-4 text-right">
                    <Button
                      ...
                      title={
                        s.products.length === 0
                          ? 'All items for this invoice have been returned'
                ...
                >
                  ...
                  <td className="px-3 py-2 text-sm text-gray-300">
                    {r.returnedProducts.map((p, idx) => (
                      <span
                        key={idx}
                        className="inline-block px-2 py-1 bg-blue-500/20 text-blue-200 rounded-md text-xs font-medium mr-2 mb-1"
                      >
      ...
      >
        {selectedSale && (
          <ReturnForm
            sale={selectedSale}
            allReturnsForSale={returns.filter(
              r => r.originalSaleId === selectedSale.id
            )}
