...
const ReportCard: React.FC<{ title: string; value: string; description?: string }> = ({
  title,
  value,
  description,
}) => (
...
/** 🔍 Extended P&L data with more details */
type ExtendedPnlAnalysisData = PnlAnalysisDataType & {
  grossRevenueBeforeReturns: number;
  netRevenueAfterReturns: number;
  returnsImpactPercentage: number;
  returnsLossList: {
    date: string;
    invoiceNumber: string;
    customerName: string;
...
const Reporting: React.FC = () => {
  ...
  const handleGenerateReport = () => {
    ...
    const filteredWholesale = allWholesale.filter(
      s => new Date(s.date) >= start && new Date(s.date) <= end
    );
    ...
    if (reportType === 'sales') {
      ...
      /** 🔍 Extra P&L metrics */
      const grossRevenueBeforeReturns = totalRevenue;
      const netRevenueAfterReturns = totalRevenue - totalReturns;
      const returnsImpactPercentage =
        totalRevenue > 0 ? (totalReturns / totalRevenue) * 100 : 0;

      /** 📋 Returns loss list (invoice-wise) */
      const returnsLossList = filteredReturns
        .map(r => {
          const warehouseName =
            allWarehouses.find(w => String(w.id) === String(r.warehouseId))?.name ??
            'Unknown';
      ...
      const dailyDataMap = new Map<
        string,
        { revenue: number; expense: number; return: number }
      >();

      [...filteredSales, ...filteredWholesale].forEach(s => {
        const entry = dailyDataMap.get(s.date) || {
          revenue: 0,
          expense: 0,
          return: 0,
        };
      ...
      filteredExpenses.forEach(e => {
        const entry = dailyDataMap.get(e.date) || {
          revenue: 0,
          expense: 0,
          return: 0,
        };
      ...
      filteredReturns.forEach(r => {
        const entry = dailyDataMap.get(r.date) || {
          revenue: 0,
          expense: 0,
          return: 0,
        };
      ...
      const extendedPnl: ExtendedPnlAnalysisData = {
        type: 'pnl',
        totalRevenue,
        totalExpenses,
        totalReturns,
        ...
        returnsImpactPercentage,
        returnsLossList,
      };

      setReportData(extendedPnl);
      ...
      filteredReturns.forEach(r => {
        const warehouseName =
          allWarehouses.find(w => String(w.id) === String(r.warehouseId))?.name ??
          'Unknown';
        const existing = warehouseMap.get(warehouseName) || {
          count: 0,
          amount: 0,
        };
        existing.count++;
      ...
      /** 🔝 Top returned products (by refund amount) */
      ...
      const warehouseData = allWarehouses.map(warehouse => {
        ...
        const totalExpenses = warehouseExpenses.reduce(
          (sum, e) => sum + e.amount,
          0
        );
      ...
      const unpaidWholesale = filteredWholesale.filter(
        s => s.status === 'Unpaid'
      );

      const retailUnpaid = unpaidRetail.reduce(
        (sum, s) => sum + s.totalAmount,
        0
      );
      const wholesaleUnpaid = unpaidWholesale.reduce(
        (sum, s) => sum + s.totalAmount,
        0
      );
  ...
  const handleExportCSV = () => {
    ...
    if (reportData.type === 'sales') {
      ...
      exportToCSV(
        [...filteredSales, ...filteredWholesale],
        `sales_report_${startDate}_to_${endDate}.csv`
      );
    } else if (reportData.type === 'pnl') {
      const filteredExpenses = allExpenses.filter(
        e => new Date(e.date) >= start && new Date(e.date) <= end
      );
      exportToCSV(
        filteredExpenses,
        `pnl_expenses_${startDate}_to_${endDate}.csv`
      );
    } else if (reportData.type === 'returns') {
      const filteredReturns = allReturns.filter(
        r => new Date(r.date) >= start && new Date(r.date) <= end
      );
      exportToCSV(filteredReturns, `returns_${startDate}_to_${endDate}.csv`);
    } else if (reportData.type === 'credits') {
      const unpaidSales = (reportData as CreditsAnalysisData).unpaidSales;
      exportToCSV(unpaidSales, `credits_${startDate}_to_${endDate}.csv`);
    } else if (reportData.type === 'warehouse') {
      const warehouseSummary = (reportData as WarehouseOverviewData).warehouses;
      exportToCSV(
        warehouseSummary,
        `warehouse_overview_${startDate}_to_${endDate}.csv`
      );
    }
  };
      ...
      {!reportData && !generating && (
        <div className="text-center py-16 bg-black/20 backdrop-blur-md border border-white/10 rounded-xl">
          <p className="text-gray-400">
            Select your filters and generate a report to see the analysis.
          </p>
        </div>
      )}
          ...
          {reportData.type === 'pnl' && (
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                />
                <ReportCard
                  title="Total Returns (Loss From Returns)"
                  value={formatCurrency(reportData.totalReturns)}
                  description="Money given back due to sales returns"
                />
                <ReportCard
                  title="Net Profit"
              ...
              {/* Extra P&L insight cards */}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <ReportCard
                  title="Gross Revenue Before Returns"
                  value={formatCurrency(reportData.grossRevenueBeforeReturns)}
                />
                <ReportCard
                  title="Net Revenue After Returns"
                  value={formatCurrency(reportData.netRevenueAfterReturns)}
                  description="Revenue after deducting sales returns"
                />
                <ReportCard
                  title="Returns Impact"
                  value={`${reportData.returnsImpactPercentage.toFixed(2)}%`}
                  description="Percentage of revenue lost due to returns"
                />
              </div>
              ...
              {/* 🔻 Loss from returns list */}
              <div className="bg-black/20 backdrop-blur-md border border-white/10 rounded-xl shadow-lg p-6">
                <h2 className="text-xl font-bold mb-4 text-white">
                  Loss From Returns – Invoice Wise
                </h2>
                {reportData.returnsLossList.length === 0 ? (
                  ...
                  <div className="overflow-x-auto">
                    <table className="min-w-full text-sm text-left text-gray-300">
                      ...
                      <tbody>
                        {reportData.returnsLossList.map(row => (
                          <tr
                            key={`${row.date}-${row.invoiceNumber}`}
            ...
            <div className="space-y-8 animate-fade-in">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <ReportCard
                  ...
                  value={formatCurrency(reportData.avgReturnValue)}
                />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                <ReportCard
                  ...
                  value={formatCurrency(
                    reportData.warehouses.reduce(
                      (acc, w) => acc + w.totalSales,
                      0
                    )
                  )}
                />
                    ...
                    <tbody>
                      {reportData.warehouses.map(w => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4 text-right">
                            {formatCurrency(w.totalInventory)}
                          </td>
                          <td
          ...
          {reportData.type === 'credits' && (
            <div className="space-y-8 animate-fade-in">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <ReportCard
                  title="Total Unpaid"
                  value={formatCurrency(reportData.totalUnpaid)}
                  description="Outstanding Amount"
                />
                <ReportCard
                  title="Retail Unpaid"
                  value={formatCurrency(reportData.retailUnpaid)}
                />
                <ReportCard
                  title="Wholesale Unpaid"
                  value={formatCurrency(reportData.wholesaleUnpaid)}
                />
              </div>

              <div className="bg-black/20 border border-white/10 rounded-xl shadow-lg overflow-hidden">
                <h2 className="text-xl font-bold p-6 text-white border-b border-white/10">
                  Unpaid Sales Details
                </h2>
                <div className="overflow-x-auto">
                  <table className="min-w-full text-sm text-left text-gray-300">
                    <thead className="bg-white/5 uppercase text-xs">
